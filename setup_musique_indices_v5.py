#!/usr/bin/env python3
"""MuSiQue v5 (corpus_idx) Index Generation Pipeline

Builds fresh v5 artifacts from the corpus_idx-aware metadata JSON.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build MuSiQue v5 indices (corpus_idx)")
    p.add_argument(
        "--metadata_json",
        type=str,
        default="MuSiQue/musique_sample_200_corpus_idx_metadata.json",
        help="Input metadata JSON (context_metadata)",
    )
    p.add_argument("--db", type=str, default="MuSiQue/metadata_v5.db", help="Output SQLite DB")
    p.add_argument(
        "--embedding_texts",
        type=str,
        default="MuSiQue/embedding_texts_v5.json",
        help="Output embedding texts JSON",
    )
    p.add_argument(
        "--embeddings",
        type=str,
        default="MuSiQue/path_embeddings_v5.npz",
        help="Output dense path embeddings (.npz)",
    )
    p.add_argument(
        "--bm25_index",
        type=str,
        default="MuSiQue/bm25_index_v5",
...
async def main() -> None:
    ...
    print("Starting MuSiQue v5 Index Generation Pipeline...")
    ...
    print("\n[OK] MuSiQue v5 indices generated successfully")
